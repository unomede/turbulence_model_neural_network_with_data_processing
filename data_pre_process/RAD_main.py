...
file_name = './original_data/RAD/strain rate and rotation tensor.dat'
...
file_name = './original_data/RAD/Reynold stresses.dat'
...
for i in range(258):
    for j in range(258):
        if ((i == 0) or (i == 257) or (j == 0) or (j == 257)) or ((i >= 65 and i <= 192) and (j >= 65 and j <= 192)):
            point_matrix[i, j] = 0
point_matrix = np.tile(point_matrix.reshape(-1, 1) > 0, (1, 6))
